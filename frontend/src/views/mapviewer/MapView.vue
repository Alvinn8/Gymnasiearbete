...
function getQueryRoom(queryString: LocationQueryValue | LocationQueryValue[]) {
    // This function attempts to be as lenient as possible in regard to both
    // rooms being renamed, and users attempting to craft custom links.
    // This is to ensure links work for as long as possible so that existing
    // links, QR-codes, etc. do not need to be replaced.
    // Third-party programs that only know of the room name may also skip
    ...
    if (index === -1) {
        ...
        if (!isNaN(id)) {
            // Only an id is specified. This probably means that the room has no name.
            // Do not validate that the room has no name since a name can be added
            // later, and old links should still work. So only check by id.
            return data.value?.rooms.find(r => r.id === id) ?? null;
        } else {
            // A custom-crafted link supplied only the room name.
            // A link like this is never generated by the official site,
            // but may be generated by third-party tools that do not
            // know the room ids.
            name = queryString;
            const matches = data.value?.rooms.filter(r => r.name === name);
            if (matches?.length === 1) {
                console.log("Link specified a room with only the room name.");
                return matches[0];
            }
            console.log("Link only specified a room name, but that room was not found.");
            return null;
        }
    } else {
    ...
    const idMatch = data.value?.rooms.find(r => r.id === id) ?? null;
    const nameMatches = data.value?.rooms.filter(r => r.name === name) ?? [];
    if (nameMatches.length === 1) {
        if (nameMatches[0] === idMatch) {
            // A room with the correct id and name was found.
            return idMatch;
        } else {
            // A room with the correct name was found, but the id did not match.
            // This is likely because the user tried to craft a custom link by
            // changing the room name, without changing the room id. Since
            // There was only one match we can return that name match.
            console.log("Link had one matching room name but an incorrect room id.");
            return nameMatches[0];
        }
    }
    if (nameMatches.length === 0 && idMatch !== null) {
        // No name match was found, but a room with the correct id was found.
        // The room has likely been renamed, return the id match.
        console.log("Link had incorrect room name, has the room been renamed?");
        return idMatch;
    }
    if (nameMatches.length > 1 && idMatch !== null) {
        // Multiple name matches were found, and we have an id match.
        // The room has to have a name, since otherwise the second
        // if statement in the function would have returned.
        // There may be multiple rooms with the same name, so return the
        // id match and hope that the user has not changed the link manually.
        console.log("Link had multiple rooms matching by name, using room id instead.");
        return idMatch;
    }
    // If we are here we have multiple name matches and no id match.
    // In such a case, we can not know which of the room matches is
    // correct.
    console.log("Link had multiple room name matches and no id match, unable to find room.");
    return null;
}
        ...
        <div class="upper">
            ...
            <template v-else>
                <SearchBar
                    ...
                    :autoFocus="selectRoomCallback !== null"
